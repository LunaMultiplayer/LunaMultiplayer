...
namespace CommonUpdater
{
    public class Downloader
    {
        ...
        private static void CopyFilesFromTempToDestination(ProductToDownload product)
        {
            ...
            foreach (var dirPath in Directory.GetDirectories($@"{FolderToDecompress}\{productFolderName}", "*", SearchOption.AllDirectories))
            {
                var destFolder = dirPath.Replace($@"{FolderToDecompress}\{productFolderName}", $@"{Directory.GetCurrentDirectory()}");
                Console.WriteLine($"Creatind dest folder: {destFolder}");
                Directory.CreateDirectory(destFolder);
            }

            foreach (var newPath in Directory.GetFiles($@"{FolderToDecompress}\{productFolderName}", "*.*", SearchOption.AllDirectories))
            {
                var destPath = newPath.Replace($@"{FolderToDecompress}\{productFolderName}", $@"{Directory.GetCurrentDirectory()}");

...
namespace LmpClient.Systems.VesselPartSyncFieldSys
{
    public class VesselPartSyncFieldMessageHandler : SubSystem<VesselPartSyncFieldSystem>, IMessageHandler
    {
        ...
        public ConcurrentQueue<VesselPartSyncFieldMsgData> StoredMessagesData { get; set; } = new ConcurrentQueue<VesselPartSyncFieldMsgData>();
        ...
        {
            LunaLog.Debug($"Current memory usage for stored messages in the VesselPartSyncField system: {Math.Floor(StoredMessagesData.Count * sizeof(VesselPartSyncFieldMsgData) / 1024)}KB");
        }
    }
}

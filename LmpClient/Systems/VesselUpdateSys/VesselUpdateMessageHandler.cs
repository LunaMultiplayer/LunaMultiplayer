...
namespace LmpClient.Systems.VesselUpdateSys
{
    public class VesselUpdateMessageHandler : SubSystem<VesselUpdateSystem>, IMessageHandler
    {
        ...
        public ConcurrentQueue<VesselUpdateMsgData> StoredMessagesData { get; set; } = new ConcurrentQueue<VesselUpdateMsgData>();
        ...
        {
            LunaLog.Debug($"Current memory usage for stored messages in the VesselUpdate system: {Math.Floor(StoredMessagesData.Count * sizeof(VesselUpdateMsgData) / 1024)}KB");
        }
    }
}
